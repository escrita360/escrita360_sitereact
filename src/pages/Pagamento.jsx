...
function Pagamento() {
  ...
  const validateForm = () => {
    ...
    if (!cardNumberClean) newErrors.cardNumber = 'Número do cartão é obrigatório'
    else if (cardNumberClean.length < 16) newErrors.cardNumber = 'Número do cartão inválido'
    
    if (!formData.cardName.trim()) newErrors.cardName = 'Nome no cartão é obrigatório'
    else if (formData.cardName.trim().split(' ').length < 2) newErrors.cardName = 'Digite o nome completo'
    ...
    else if (phoneClean.length < 10) newErrors.phone = 'Telefone inválido'
